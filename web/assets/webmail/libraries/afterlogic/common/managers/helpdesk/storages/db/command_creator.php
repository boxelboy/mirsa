...
class CApiHelpdeskCommandCreator extends api_CommandCreator
{
	...
	{
		return sprintf('SELECT email FROM %sahd_users WHERE id_tenant = %d AND mail_notifications = 1 AND is_agent = 1',
//		return sprintf('SELECT email FROM %sahd_users WHERE id_tenant = %d AND is_agent = 1',
...
WHERE %s = %s AND %s = %s%s';
	...
	{
		$sSql = 'SELECT id_helpdesk_user, email, name, is_agent FROM %sahd_users WHERE %s = %d AND %s IN (%s)';
	...
	public function GetThreads(CHelpdeskUser $oHelpdeskUser, $iOffset = 0, $iLimit = 20, $iFilter = EHelpdeskThreadFilterType::All, $sSearch = '', $iSearchOwner = 0)
	{
		...
		$sSql .= ' ORDER BY updated desc LIMIT '.((int) $iOffset).', '.((int) $iLimit);
