...
class CApiUsersCommandCreator extends api_CommandCreator
{
     ...
     * @return string
     */
    public function CreateAUser($id = null)
    {
        if ($id) {
            $sSql = 'INSERT INTO %sa_users ( id_user, deleted ) VALUES ( %s, 0 )';
            return sprintf($sSql, $this->Prefix(), $id);
        } else {
...
AND %sacal_calendars.user_id = %d';
	 ...
	 * @return string
	 */
	public function AccountExists($sEmail, $sLogin, $sUserId, $niExceptAccountId = null)
	{
...
WHERE def_acct = 1 AND %s = %s AND %s = %s AND %s = %s %s';
		...
		return sprintf(trim($sSql), $this->Prefix(),
			...
			$this->escapeColumn('id_user'), $sUserId,
			$sAddSql);
	}
	...
	{
		$sSql = 'SELECT id_acct, email, def_acct, friendly_nm, signature, signature_opt, signature_type FROM %sawm_accounts WHERE id_user = %d';
...
WHERE def_acct = 1 AND id_domain = %d%s ORDER BY %s %s LIMIT %d, %d';
	...
	public function AccountExists($sEmail, $sLogin, $sUserId, $niExceptAccountId = null)
	{
		return parent::AccountExists($sEmail, $sLogin, $sUserId, $niExceptAccountId).' LIMIT 1';
	}
