...
class CApiUsersDbStorage extends CApiUsersStorage
{
	...
	public function __construct(CApiGlobalManager &$oManager)
	{
		...
		$this->oCommandCreator =& $oManager->GetCommandCreator(
			$this, array(EDbType::MySQL => 'CApiUsersCommandCreatorMySQL', EDbType::SQLite => 'CApiUsersCommandCreatorSQLite')
		);
	}
    ...
    {
        if (!$this->oConnection->Execute($this->oCommandCreator->CreateAUser($oUser->IdUser))) {
            return false;
        }

        return $this->oConnection->Execute($this->oCommandCreator->CreateUser($oUser));
    }
	...
	public function AccountExists(CAccount $oAccount)
	{
		...
		if ($this->oConnection->Execute($this->oCommandCreator->AccountExists(
			$oAccount->Email, $oAccount->IncomingMailLogin, $oAccount->IdUser, (0 < $oAccount->IdAccount) ? $oAccount->IdAccount : null)))
		{
			$oRow = $this->oConnection->GetNextRecord();
