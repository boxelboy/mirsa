...
class CDbPdoMySql extends CDbSql
{
	 ...
	 */
	public function __construct($sHost, $sUser, $sPassword, $sDbName)
	{
	...
	public function Connect($bWithSelect = true, $bNewLink = false)
	{
		...
		if (class_exists('PDO'))
		{
			try
			{
				...
				$this->oPDO = @new PDO('mysql:'.implode(';', $aParts), $sDbLogin, $sDbPassword, $aPDOAttr);
				if (CApi::$bUseDbLog)
				{
	...
	public function GetTableFields($sTableName)
	{
		...
		{
			if ($oValue && $oValue->Field && 0 < strlen($oValue->Field))
			{
	...
	public function GetTableIndexes($sTableName)
	{
		...
		while (false !== ($oValue = $this->GetNextRecord()))
		{
			if ($oValue && $oValue->Key_name && $oValue->Column_name)
			{
